...
"""HammerCloud core URL dispatcher configuration."""


urlpatterns = patterns('',
    url(r'^accounts/login/$',  'django.contrib.auth.views.login', {'template_name': 'core/app/login.html'}, name='login-view'),
    url(r'^accounts/logout/$', 'hc.core.base.views.control.logout',                                         name='logout-view'),
)


urlpatterns += patterns('django.views.generic.simple',
    url('^app/(?P<app>[a-z]+)/admin/$', RedirectView.as_view(url='/hc/admin/%(app)s/')),
)
...
# All the views are served by the dispatcher, which receives the function name
# to execute per view, which will be resolved by each app in runtime.
# TODO(rmedrano): Looks lame to have the word 'dispatcher' repeaded 100 times.
urlpatterns += patterns('hc.core.base.views.dispatcher',
    # Index.
    url(r'^app/(?P<app>[a-z]+)/$', 'dispatcher', name='index-view', kwargs={'func': 'index'}),

    # "More" HC (views for different models).
    ...
    url(r'^app/(?P<app>[a-z]+)/ajax/nightly/$',                                                    'dispatcher', name='ajaxnightly-view',       kwargs={'func': 'ajaxnightly'}),
    ...
    url(r'^app/(?P<app>[a-z]+)/robot/nightly/$',                                                'dispatcher', name='nightly-view',                      kwargs={'func': 'nightly'}),
    url(r'^app/(?P<app>[a-z]+)/autoexclusion/control/$',                                        'dispatcher', name='autoexclusion_control-view',        kwargs={'func': 'autoexclusion_control'}),
    url(r'^app/(?P<app>[a-z]+)/autoexclusion/control/(?P<action>[a-z]+)/$',                     'dispatcher', name='autoexclusion_control_action-view', kwargs={'func': 'autoexclusion_control_action'}),
    ...
    # TODO(rmedrano): for God's sake, optimize this regexp.
    ...
    url(r'^app/(?P<app>[a-z]+)/reports/$',     'dispatcher', name='reports-view',     kwargs={'func': 'reports'}),

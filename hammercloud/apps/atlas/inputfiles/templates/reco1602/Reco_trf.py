...
from PyJobTransformsCore.full_trfarg import NbOutputDPDArg
...
class FlexibleRecoTransform( BaseOfCompositeTrf ):
    def __init__(self, inDic):
        ...
        self.dicRAWToRAWHLT=self.AddNewSubStep("r2r")
        self.dicRAWToESD=self.AddNewSubStep("r2e")
        self.dicRDOToBS=self.AddNewSubStep("r2b")
        self.dicESDToESD=self.AddNewSubStep("e2e")
        self.dicESDToAOD=self.AddNewSubStep("e2a")
        self.dicESDToDPD=self.AddNewSubStep("e2d")
        self.dicAODToDPD=self.AddNewSubStep("a2d")
        self.dicAODToTAG=self.AddNewSubStep("a2t")
    ...
    def runJob(self):
        ...
        ########################
        # RDO->BS
        if (allOK and self.runRDOtoBS()):
            dic=self.dicRDOToBS.copy()
            ## hack 1 ####
            import os
            bs_out=''
            print 'bs name %s' % dic['outputBSFile']
            if dic.has_key('outputBSFile'):
                bs_out=dic['outputBSFile']
                dic.update({'outputBSFile':'RAW._0001.data'})
            ##############       
            ...
            ## hack 2 ##
            if bs_out:          
                try: os.rename("RAW._0001.data",bs_out)             
                except: print "RAW._0001.data doesn't exist. It's probably OK for a skimmed bytesetream."
                dic.update({'outputBSFile':bs_out})
            ###########(hacks will be removed soon)     
            print "bsOK is ",bsOK
        ...
        ########################
        # RAW->ESD
        if (allOK and self.runRAWtoESD()):
            dic=self.dicRAWToESD.copy()
            
##             #Hacks needed while waiting for TDAQ update
##             draw_zee_out=''
##             if dic.has_key('outputDRAW_ZEEFile'):
##                 draw_zee_out=dic['outputDRAW_ZEEFile']
##                 dic.update({'outputDRAW_ZEEFile':'DRAW_ZEE._0001.data'})
##             draw_wenu_out=''
##             if dic.has_key('outputDRAW_WENUFile'):
##                 draw_wenu_out=dic['outputDRAW_WENUFile']
##                 dic.update({'outputDRAW_WENUFile':'DRAW_WENU._0001.data'})
##             draw_zmumu_out=''
##             if dic.has_key('outputDRAW_ZMUMUFile'):
##                 draw_zmumu_out=dic['outputDRAW_ZMUMUFile']
##                 dic.update({'outputDRAW_ZMUMUFile':'DRAW_ZMUMU._0001.data'})
##             draw_wmunu_out=''
##             if dic.has_key('outputDRAW_WMUNUFile'):
##                 draw_wmunu_out=dic['outputDRAW_WMUNUFile']
##                 dic.update({'outputDRAW_WMUNUFile':'DRAW_WMUNU._0001.data'})
          ...
          ##   if draw_zee_out:
##                 import os
##                 try: os.rename('DRAW_ZEE._0001.data',draw_zee_out)
##                 except: print "DRAW_ZEE._0001.data doesn't exist. It's probably OK for a skimmed bytesetream."
##             if draw_wenu_out:
##                 import os
##                 try: os.rename('DRAW_WENU._0001.data',draw_wenu_out)
##                 except: print "DRAW_WENU._0001.data doesn't exist. It's probably OK for a skimmed bytesetream."
##             if draw_zmumu_out:
##                 import os
##                 try: os.rename('DRAW_ZMUMU._0001.data',draw_zmumu_out)
##                 except: print "DRAW_ZMUMU._0001.data doesn't exist. It's probably OK for a skimmed bytesetream."
##             if draw_wmunu_out:
##                 import os
##                 try: os.rename('DRAW_WMUNU._0001.data',draw_wmunu_out)
##                 except: print "DRAW_WMUNU._0001.data doesn't exist. It's probably OK for a skimmed bytesetream."
    ...
    def ConfigureInternalSubSteps(self):
        ...
        #Final hack for to separate RDO->BS->ESD and RDO->ESD
    ...
    def AddDPD(self,dpdName):
        #This dummy function is not explicitely used by Reco_trf
        #It still needs to be defined to be compatible with ConfigDic
        print "WARNING Reco_trf AddDPD() is obsolete and not expected to be used."
        return

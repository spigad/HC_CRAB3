...
Analysis::CalibrationDataInterfaceROOT::CalibrationDataInterfaceROOT(const string& taggerName, string configname, string pathname)
{
  ...
  test = testPrefix; test += "EfficiencyCalibrationTauName";
  string calibrationTauName(env.GetValue(test.c_str(), "default"));
							...
							const string& OP, Uncertainty unc) const
{
  ...
  // temporary hack: for taus, use the scale factors for charm jets but with doubled uncertainties
  if (label == "T") {
    Analysis::CalibResult result = getScaleFactor(variables, "C", OP, unc);
    result.second *= 2.;
    return result;
  }
  ...
  */
  ...
  Analysis::CalibResult result = getResults(x, xsyst, fct, fctsys, cov);
...
Analysis::CalibrationDataInterfaceROOT::getEfficiency (const CalibrationDataVariables& variables,
						       ...
						       const string& OP, Uncertainty unc) const
{
  Analysis::CalibResult sfResult = getScaleFactor(variables, label, OP, unc);
								   ...
								   const string& OP, Uncertainty unc) const
{
  ...
  Analysis::CalibResult effResult = this -> getMCEfficiency(variables, label, OP, unc);
  Analysis::CalibResult sfResult = this -> getScaleFactor(variables, label, OP, unc);
...
void
Analysis::CalibrationDataInterfaceROOT::retrieveFunctionArguments (const CalibrationDataVariables& variables,
...
TObject* Analysis::CalibrationDataInterfaceROOT::retrieveTObject(const string& name, bool eff) const
{
...
string
Analysis::CalibrationDataInterfaceROOT::fullName(const string& author, const string& name) const
{

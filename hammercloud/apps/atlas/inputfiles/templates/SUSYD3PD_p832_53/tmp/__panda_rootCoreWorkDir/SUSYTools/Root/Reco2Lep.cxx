...
//									MET_Simplified20_RefJet_ety_ForwardReg;
        ...
        // 2012-01-09: Renaud, dump dummy values => Need to be fixed !!!!!
        std::vector<float> vdummyfloat;
        std::vector< std::vector<float> > vdummyfloat2;
        std::vector< std::vector<unsigned int> > vdummyuint;
        std::vector<int> vdummyint;
	TVector2 met = susyobj.GetMET(&vdummyfloat,&vdummyfloat,&vdummyfloat,&vdummyfloat,&vdummyfloat2,&vdummyfloat2,&vdummyfloat2,&vdummyuint,vdummyint,&vdummyfloat2,&vdummyfloat2,&vdummyfloat2,&vdummyuint,0.,0.,0.,0.,0.,0.,vdummyint,&vdummyfloat,&vdummyfloat,&vdummyfloat,&vdummyfloat);
...
vector<int> Reco2Lep::get_electron_baseline(int myRunNumber){
	vector<int> elec_index;
        // 2012-01-09: Fix from Renaud add a number 1. corresponding to el_wet
	for(int ielec=0;ielec<el_n;ielec++){
		if (susyobj.IsElectron(ielec,myRunNumber, 
					...
					el_author->at(ielec), el_isEM->at(ielec), el_OQ->at(ielec), el_nPixHits->at(ielec), el_nSCTHits->at(ielec), 1.,
...
vector<int> Reco2Lep::get_jet_good(vector<int> elec_index, int &nbadjets){
	...
	for(int ijet=0; ijet<jet_AntiKt4TopoNewEM_n; ijet++){
		...
		bool goodjet = susyobj.IsJet(ijet, jet_AntiKt4TopoNewEM_pt->at(ijet),
					     ...
					     jet_AntiKt4TopoNewEM_flavor_truth_label->at(ijet),

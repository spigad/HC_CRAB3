...
APReweight::APReweight(TTree* denominator, string denominator_branch, TTree* numerator, string numerator_branch, unsigned int n_bins, double x_min, double x_max, bool isTrig) { /*!< Constructor which takes the object to be reweighted, the object container to perform the reweighting on and the corresponding efficiency map. The reweighting will be performed in all dimensions contained in the EfficiencyMap. */
...
APReweight::APReweight(vector<double> denominator, vector<double> numerator, unsigned int n_bins, double x_min, double x_max, bool isTrig) { /*!< Constructor which takes the object to be reweighted, the object container to perform the reweighting on and the corresponding efficiency map. The reweighting will be performed in all dimensions contained in the EfficiencyMap. */
  //Prepare simple histograms to store values for current binning
  _empty_weight = new APWeightEntry(0, 0, 1.);
  _denominator_hist = new TH1D("", "denominator_hist", n_bins, x_min, x_max);
  _numerator_hist = new TH1D("", "numerator_hist", n_bins, x_min, x_max);
...
APReweight::APReweight(TH1* denominator_in, TH1* numerator_in, bool isTrig) { /*!< Constructor which takes the object to be reweighted, the object container to perform the reweighting on and the corresponding efficiency map. The reweighting will be performed in all dimensions contained in the EfficiencyMap. */
  //Prepare simple histograms to store values for current binning
  _empty_weight = new APWeightEntry(0, 0, 1.);

...
//    float stat_err_d = histo_drellyan->GetBinError(i);
  ...
  // add luminosity error (11%)
  // dlumi = 0.2 * lumi
  // nevts_renorm = nevts * lumi
  // d_nevts_renorm = nevts * dlumi + dnevts * lumi

    float dlumi = 0.11; //because I have here already renormalized events...
...
//    float lumi_err_d = dlumi * histo_drellyan->GetBinContent(i);
  ...
  // add MC error (60% for W and Z)

    float mc_err_w = .6 * histo_wjets->GetBinContent(i);
    float mc_err_z = .6 * histo_zjets->GetBinContent(i);

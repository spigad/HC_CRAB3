...
class ProductionBlacklist:
  ...
  def send_cloud_online_alert(self, site):
    ...
    if self.debug:
        to = self.dan
    subject = "[HC-PFT-dummy] %s reset online at %s CET" % (site, time.ctime())
    if self.debug:
        subject += ' DEBUG'
    body = "Dear %s,\n\n" % cloud_support
  ...
  def send_cloud_alert(self, site):
    ...
    if self.debug:
        to = self.dan
    subject = "[HC-PFT-dummy] %s Auto-Excluded at %s CET" % (site, time.ctime())
    if self.debug:
        subject += ' DEBUG'
    body = "Dear %s,\n\n" % cloud_support
  ...
  def send_alert(self):
    if not self.log:
        return
    to = self.daexp
    subject = "[HC-PFT-dummy] Production Blacklisting Report at %s CET" % time.ctime()
    if self.debug:
        to = self.dan
        subject += ' DEBUG'
